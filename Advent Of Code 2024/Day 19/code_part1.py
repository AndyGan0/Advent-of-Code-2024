...
def TrycreatePatterns(can_be_made: np.array):
    ...
    while (len(help_dict[0]) != 0 and (can_be_made.shape[0] - 1) not in help_dict[0]):
        for j in list(help_dict[0]):
            help_dict[0].pop(j)
            if (j+1) in help_dict:
                for next in help_dict[j+1]:
                    help_dict[0][next] = None


    if (0 not in help_dict):
        return False

    return (can_be_made.shape[0] - 1) in help_dict[0]
...
for desired in desired_patterns:
    ...
    result = TrycreatePatterns(can_be_made)
    if (result):
        count += 1
