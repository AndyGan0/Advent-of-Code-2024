...
for freq in differentFreqNodes.keys():
    for i in range(len(differentFreqNodes[freq])):
        for j in range(len(differentFreqNodes[freq])):
            if (i != j):
                antinode_diff_i, antinode_diff_j = findAntinode(differentFreqNodes[freq][i], differentFreqNodes[freq][j])
                antinode_i, antinode_j = differentFreqNodes[freq][i]
                while (antinode_i >= 0 and antinode_j >= 0 and antinode_i < locationsWithAthinodes.shape[0] and antinode_j < locationsWithAthinodes.shape[1]):
                    locationsWithAthinodes[antinode_i, antinode_j] = "#"
                    antinode_i += antinode_diff_i
                    antinode_j += antinode_diff_j
